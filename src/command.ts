...
export function runShell(command: string, cwd: string, timeout?: number, shell?: string): Promise<CommandResult> {
  return runProcess({ program: command, shell: shell ?? true }, cwd, timeout);
}

export function runExec(program: string, args: string[] = [], cwd = process.cwd(), timeout?: number): Promise<CommandResult> {
  return runProcess({ program, args }, cwd, timeout);
}

async function runProcess(spec: ProcessSpec, cwd: string, timeout = 10 * 60_000): Promise<CommandResult> {
  return new Promise((resolve) => {
    ...
    child.stdout.on("data", (data: Buffer) => { stdout += data.toString(); process.stdout.write(data); });
