...
When a listed workflow matches the task, prefer the flow tools: after creating or editing a flow, use validate_flow; then use run_flow with its declared inputs, and inspect_flow_run for details or failures that the bounded summary omits. Use direct shell or file tools for trivial one-off work or when no relevant flow exists.\n\n${formatFlowCatalog(catalog)}\nPass values through declared workflow inputs. Use validate_flow after creating or editing a flow. Treat flow results as evidence and do not infer omitted details; inspect a saved run when exact evidence is needed.` };
  });
