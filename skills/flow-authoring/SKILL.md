...
6. Every selected agent must have effective `model`, `thinkingLevel`, and `prompt` declarations. Variants may inherit common values from their parent, but must provide any field the parent omits; choose levels deliberately and remember Pi may clamp them to model capabilities.
...
8. Use `exec` for direct commands and `shell` only when shell syntax is required. Use a unique, non-empty named group such as `parallel: evidence` only for independent, read-only steps; preserve barriers before dependent steps. To fan out from common agent evidence, retain it with `context` in an earlier seed agent and give each child a compatible `forkContext`; forks cannot depend on a same-batch seed and never share a mutable session.
